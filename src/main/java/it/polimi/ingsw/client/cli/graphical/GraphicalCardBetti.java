...
public class GraphicalCardBetti {
    ...
    public GraphicalCardBetti(GraphicalDevelopmentCardGrid graphicalGrid, LightDevelopmentCard ldc) {
    ...
    private void drawProductionCost(int x_coord, int y_coord) {
        drawCost(x_coord+2, y_coord+1, 3, lightDevelopmentCard.getProductionCost());
    ...
    private void drawVictoryPoints(int x_coord, int y_coord) {
        ...
        graphicalGrid.addPixel(x_coord+4, y_coord+h_center-2, Colour.ANSI_BRIGHT_YELLOW, 'V');
        graphicalGrid.addPixel(x_coord+4, y_coord+h_center-1, Colour.ANSI_BRIGHT_YELLOW, 'P');
        graphicalGrid.addPixel(x_coord+4, y_coord+h_center, Colour.ANSI_BRIGHT_WHITE, ':');
        if(vp > 9)
            graphicalGrid.addPixel(x_coord+4, y_coord+h_center+1, Colour.ANSI_BRIGHT_YELLOW, String.valueOf(vp/10).charAt(0));
        graphicalGrid.addPixel(x_coord+4, y_coord+h_center+2, Colour.ANSI_BRIGHT_YELLOW, String.valueOf(vp%10).charAt(0));
    }

    private void drawID(int x_coord, int y_coord) {
        int ID = lightDevelopmentCard.getID();
        if(ID>9)
            graphicalGrid.addPixel(x_coord+1, y_coord+CardWidth-3, Colour.ANSI_BRIGHT_WHITE, SubscriptNumbers.valueOf(ID/10).getCode().charAt(0));
        graphicalGrid.addPixel(x_coord+1, y_coord+CardWidth-2, Colour.ANSI_BRIGHT_WHITE, SubscriptNumbers.valueOf(ID%10).getCode().charAt(0));
    }

    private void drawCost(int x_coord, int y_coord, int posix, List<String> cost) {
        for (int j = 0; j < cost.size(); j++){
            if(j%2==0)
                graphicalGrid.addPixel(x_coord+posix, y_coord+j+6, Colour.ANSI_BRIGHT_WHITE, cost.get(j).charAt(0));
            else {
                ...
                graphicalGrid.addPixel(x_coord + posix, y_coord + j+6, c, r.symbol.charAt(0));
            }
            if(posix == 3){
